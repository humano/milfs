...
if (mysql_num_rows($sql)!='0'){
	...
	while( $row = mysql_fetch_array( $sql ) ) {
		...
		elseif($campo_tipo=='14'){
			...
			<img class=' img-responsive'  style='width:100%'  src='http://api.tiles.mapbox.com/v4/examples.map-zr0njcqy/url-http%3A%2F%2F$_SERVER[HTTP_HOST]%2Fmilfs%2Fimages%2Ficonos%2Fpin.png($lat,$lon,$zoom)/$lat,$lon,$zoom/600x250.png?access_token=pk.eyJ1IjoiYmlibGlvbGFicyIsImEiOiJjaWVobmJnNXIwMGQ3c3JtMW55dzYwYnF3In0.q4BDrWo28H6fazZjbBKVtQ' >
			"; 
										}
			}
	...
	while( $row = mysql_fetch_array( $sql ) ) {
		...
		$array[$row[id_campo]] = $contenido;	
			...
			<!-- <img class='img-round'  src='http://dev.openstreetmap.de/staticmap/staticmap.php?center=$lon,$lat&zoom=$zoom&size=350x100&maptype=mapnik&markers=$lon,$lat,red-pushpin' > -->
						<img class='img-round '  src='http://api.tiles.mapbox.com/v4/examples.map-zr0njcqy/url-http%3A%2F%2F$_SERVER[HTTP_HOST]%2Fmilfs%2Fimages%2Ficonos%2Fnaranja.png($lat,$lon,$zoom)/$lat,$lon,$zoom/350x100.png?access_token=pk.eyJ1IjoiYmlibGlvbGFicyIsImEiOiJjaWVobmJnNXIwMGQ3c3JtMW55dzYwYnF3In0.q4BDrWo28H6fazZjbBKVtQ' >";
											} else { $contenido ='';}
			}
			elseif($campo_tipo=='4'){ $contenido = "<a href='$contenido' target='_blank'>$contenido</a>";}
